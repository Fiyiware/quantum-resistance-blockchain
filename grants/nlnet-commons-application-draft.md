...
- **Second Rust PoC binding that core to the real EVM precompile interface** (`poc/evm-precompile/`): the verifier is wrapped as a `revm` 41 `Precompile` (the EVM engine Reth uses) with real input/output ABI, linear gas accounting and a fixed address. An end-to-end test stands up a full `revm` EVM with a custom `PrecompileProvider` registering `MLDSAVERIFY` at `0x101` and sends a **real transaction** that calls it — the ML-DSA-65 verification runs through the entire transaction pipeline and returns the expected boolean. This substantially de-risks Phase 1 deliverable 1: the EVM-level mechanism is demonstrated; wiring the same provider into a shipping Reth node behind JSON-RPC with a faucet is the remaining (funded) engineering, not an open question.
