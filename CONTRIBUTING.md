...
Python: stdlib + `dilithium-py` only for the prototype core. No new dependencies without an issue first. Format with `black` (default config), lint with `ruff` (default config). CI enforces both on every push and PR.
